...
class Lumberjacks(gym.Env):
    ...
    Args:
        ...
        n_obstacles: number of randomly placed obstacles
    ...
    def __init__(self, tree_strength, grid_shape: Coordinates = (5, 5), n_agents: int = 2, n_trees: int = 12,
                 agent_view: Tuple[int, int] = (1, 1), full_observable: bool = False,
                 step_cost: float = -1, tree_cutdown_reward: float = 10, max_steps: int = 100, n_obstacles: int = 3,max_steps_without_reward: int = 2000,
                 ):
        
        print("Running Ming Dec10 Version")#random

        assert 0 < n_agents
        assert n_agents + n_trees <= np.prod(grid_shape)
        assert 1 <= agent_view[0] <= grid_shape[0] and 1 <= agent_view[1] <= grid_shape[1]
        assert n_trees == len(tree_strength)
        ...
        self.n_obstacles = n_obstacles # Number of randomly place obstacles
        ...
        self.max_steps_without_reward = max_steps_without_reward
        ...
        self._tree_strength = tree_strength

        mask_size = np.prod(tuple(2 * v + 1 for v in self._agent_view))
        # Agent ID (1) + Pos (2) + Step (1) + Neighborhood (3(agents,trees,obstacles) * mask_size)
        self._obs_len = (1 + 2 + 1 + 3 * mask_size) # Changed from 2 to 3, since new layer has 3
    ...
    def _init_episode(self):
        ...
        agent_id, tree_id,obstacles_id = 0, self.n_agents,0
        self._agents = []
        ...
        for pos, cell in np.ndenumerate(init_positions):
            ...
            if cell == PRE_IDS['agent']:
                ...
                self._tree_map[pos] = self._tree_strength[index] # This is where you can set the relative strength of the trees
                #self._tree_map[pos] = 1#2#self.np_random.randint(1, self.n_agents + 1) # Generate a random strength, maybe change this
                index += 1
                tree_id += 1
            elif cell == PRE_IDS['wall']:
                self._obstacles_map[pos] = 1
                obstacles_id += 1
    ...
    def _generate_init_pos(self) -> np.ndarray:
        ...
        # These initial positions will be sampled from to put in teh squares
        init_pos = np.array(
            [PRE_IDS['agent']] * self.n_agents +
            [PRE_IDS['tree']] * self._n_trees +
            [PRE_IDS['wall']] * self.n_obstacles +
            [PRE_IDS['empty']] * (np.prod(self._grid_shape) - self.n_agents - self._n_trees - self.n_obstacles)
        )
        ...
        # Removed the shuffle again to save computational speed, since we're not overly concerened
        # with getting an exactly new position each time
    ...
    def render(self, mode='human'):
        ...
        for pos, agent_strength, tree_strength, obstacles in self._view_generator(mask):
            if tree_strength and agent_strength:
                cell_size = (CELL_SIZE, CELL_SIZE / 2)
                tree_pos = (pos[0], 2 * pos[1]) # These are only the pos for the text spacing
                agent_pos = (pos[0], 2 * pos[1] + 1)
            else:
                cell_size = (CELL_SIZE, CELL_SIZE)
                tree_pos = agent_pos = obstacles_pos = (pos[0], pos[1])
            ...
            if obstacles != 0:
                fill_cell(img, pos=obstacles_pos, cell_size=cell_size, fill=WALL_COLOR, margin=0.1)
                write_cell_text(img, text=str('w'), pos=(pos[0], 2 * pos[1]),
                                cell_size=cell_size, fill='white', margin=0.1)
    ...
    def _view_generator(self, mask: Tuple[slice, slice]) -> Tuple[Coordinates, int, int]:
        ...
        obstacle_iter = np.nditer(self._obstacles_map[mask]) 
        for (pos, n_a), n_t,n_obstacles in zip(agent_iter, tree_iter,obstacle_iter):
            yield pos, n_a, n_t, n_obstacles
    ...
    def get_agent_obs(self) -> List[List[float]]:
        ...
        for i, (agent_id, agent) in enumerate(self._agent_generator()):
            ...
            for j, (_, agent_strength, tree_strength, obstacles) in zip(
                    itertools.count(start=4, step=3), # Step = 2 before
    ...
    def step(self, agents_action: List[int]):
        ...
        # Calculate rewards Ming change
        agent_masked = self._agent_map[mask] 
        if agent_masked.size != 0: # Added by Ming # Now, if two agents cut down a tree, they both get 20 points
            tree_values = self._tree_map[mask]
            rewards += np.sum(agent_masked*self._tree_cutdown_reward*tree_values.reshape(-1,1),axis=0) # Added by Ming

        # Calculate rewards Original
        #rewards_from_trees = np.sum(mask * self._tree_cutdown_reward, axis=(0, 1))
        #rewards += rewards_from_trees
      ...
      #  if rewards_from_trees == 0: # no reward this round
      #      self.steps_without_reward += 1
      #      if self.steps_without_reward >= self.max_steps_without_reward:
      #          self._agent_dones = [True] * self.n_agents
      #          print("finished early")
      #  else: self.steps_without_reward = 0 # reward this round
    ...
    def _next_pos(self, curr_pos: Coordinates, move: int) -> Coordinates:
        ...
        if self._obstacles_map[next_pos[0],next_pos[1]] != 0: # Do a check here to stop the agent from moving if this is the case
            next_pos = curr_pos

        return (
            min(max(next_pos[0], self._agent_view[0]), self._grid_shape[0]+self._agent_view[0]-1), # Used to be -1
            min(max(next_pos[1], self._agent_view[1]), self._grid_shape[1]+self._agent_view[1]-1), # Used to be -1
        )
