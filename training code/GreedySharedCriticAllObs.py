...
class QNet(nn.Module):
    ...
    def __init__(self, observation_space, action_space,weight_multiplier = 1):
        ...
        total_obs = int(sum([_.shape[0] for _ in observation_space]) / self.num_agents)
    ...
    def forward(self, obs, action): # Returns the q value evaluations for the agent possible actions
        ...
        for agent_i in range(self.num_agents):
            obs_agent = obs[:,agent_i,:].unsqueeze(1)
            x = torch.cat((obs_agent.view(obs_agent.shape[0], obs_agent.shape[1] * obs_agent.shape[2]),
...
def main(env_name, lr_mu, lr_q, tau, gamma, batch_size, buffer_limit, max_episodes, log_interval, test_episodes,
    ...
    #q_target.load_state_dict(torch.load('/Users/mingliu/Documents/R Learning/Final Project Code/all_fixed_all_obs/q_target_ALL_OBS_agents3_grid6.pt'))
    ...
    #mu_target.load_state_dict(torch.load('/Users/mingliu/Documents/R Learning/Final Project Code/all_fixed_all_obs/mu_target_ALL_OBS_agents3_grid6.pt'))
...
#-- greedy
            ...
            if episode_i % render_interval == 0: # Render the tests
                ...
                #test(test_env, 5, mu, n_agents,render=True)
                print('Saved q and mu targets')
                torch.save(q_target.state_dict(),'G_q_target_ALL_OBS_agents{}_grid{}.pt'.format(n_agents,grid_size))
                torch.save(mu_target.state_dict(),'G_mu_target_ALL_OBS_agents{}_grid{}.pt'.format(n_agents,grid_size))
...
if __name__ == '__main__':
    ...
    if USE_WANDB:
        import wandb
        wandb.init(project='Greedy_Agent{}Size{}'.format(n_agent,mapsize), config={'type':'ALLOBSFIXED','algo': 'maddpg', **kwargs, }, monitor_gym=True)
